...
namespace spiritsaway::meta::language
{
		...
		node_base(_in_node)
	{
		...
		if (cur_cursor.kind == CXCursorKind::CXCursor_ClassTemplate)
		{
			...
			m_decl_type = type_db::instance().get_type_for_template_class(cur_cursor);
		}
		else
		{
			m_decl_type = type_db::instance().get_type(cur_cursor_type);
		}
		...
		if (m_decl_type)
		{
			the_logger.info("class {} has type {}", name(), m_decl_type->name());
			m_decl_type->set_related_class(this);
		}
		else
		{
	...
	void class_node::parse()
	{
		...
		for (const auto& i : children)
		{
			...
			case CXCursor_VarDecl:
			{
				...
				auto cur_var_node = new variable_node(node_db::get_instance().create_node(i));
				auto cur_var_name = cur_var_node->name();
				auto pre_iter = m_static_fields.find(cur_var_name);
				if (pre_iter != m_static_fields.end())
				{
				...
				m_static_fields[cur_var_name] = cur_var_node;
				break;
			}
			case CXCursor_CXXMethod:
			{
				...
				{
					m_static_methods.emplace(cur_func_name, cur_func_node);
				}
				else
				{
					m_methods.emplace(cur_func_name, cur_func_node);
				}
				break;
			}
			...
			case CXCursor_TemplateTypeParameter:
			{
				...
				m_template_args.push_back(utils::to_string(i));
	...
	{
		auto _candidates = m_methods.equal_range(_func_name);
	...
	{
		auto _candidates = m_static_methods.equal_range(_func_name);
		auto candi_begin = _candidates.first;
		auto candi_end = _candidates.second;
		while (candi_begin != candi_end)
		{
	...
	{
		auto cur_iter = m_static_fields.find(_field_name);
		if (cur_iter == m_static_fields.end())
		{
			return nullptr;
		}
		else
	...
	const callable_node* class_node::has_constructor_for(const std::vector<const type_info*>& _args) const
	{
		...
		for (const auto & one_item : m_bases)
		{
	...
	json class_node::to_json() const
	{
		...
		for (const auto i : m_static_fields)
		{
		...
		for (const auto& i : m_static_methods)
		{
		...
		result["template_args"] = m_template_args;
